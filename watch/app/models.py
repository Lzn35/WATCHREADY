...
def get_philippines_time():
	"""Get current time in Philippines timezone - simplified to reduce memory usage"""
	# Use UTC + 8 hours for Philippines time (simpler than pytz)
	return datetime.utcnow() + timedelta(hours=8)

class TimestampMixin:
	created_at = db.Column(db.DateTime, default=get_philippines_time, nullable=False)
	updated_at = db.Column(
		db.DateTime, default=get_philippines_time, onupdate=get_philippines_time, nullable=False
	)
...
class AuditLog(db.Model):
	...
	id = db.Column(db.Integer, primary_key=True)
	timestamp = db.Column(db.DateTime, default=get_philippines_time, nullable=False)
	...
	def log_activity(action_type, description, user_id=None, ip_address=None, user_agent=None):
		"""Helper method to create audit log entries with Philippines timezone"""
		log_entry = AuditLog(
