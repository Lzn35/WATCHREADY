...
def delete_user(user_id):
	...
	try:
		...
		# Set deleted_by_id to NULL in persons table (preserve soft-delete history)
		db.session.execute(
			db.text("UPDATE persons SET deleted_by_id = NULL WHERE deleted_by_id = :user_id"),
			{"user_id": user_id}
		)
		
		# Set deleted_by_id to NULL in cases table (preserve soft-delete history)
		db.session.execute(
			db.text("UPDATE cases SET deleted_by_id = NULL WHERE deleted_by_id = :user_id"),
			{"user_id": user_id}
		)
