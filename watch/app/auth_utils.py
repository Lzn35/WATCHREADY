...
def user_read_create_only(f):
    """Decorator to restrict user role to create and read only (no edit/delete)"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session:
            # Not logged in
        ...
        if user.is_user():
            # Check if this is an edit/delete operation
            if request.method in ['POST', 'PUT', 'DELETE']:
                # Check if it's an edit/delete operation by looking at the route
                if any(keyword in request.endpoint for keyword in ['edit', 'delete', 'update', 'remove']):
                    flash('Access Denied. You can only create and view records. Edit/Delete operations are restricted to administrators.', 'error')
                    return redirect(url_for('core.dashboard'))
...
def load_user():
    ...
    try:
        ...
        # If database query fails, set current_user to None
...
def login_user(user):
    ...
    session.permanent = False  # Session expires when browser closes for security
